...
<body>
    <div class="wrapper">
        ...
        <h4 class="noto-bold" style="line-height: 30px">강의 검색 <span style="font-size: 16px">(<?=$_SESSION["phone"]?> | <a href="logout.php">로그아웃</a>)</span></h4>
        <div class="login" style="margin: 80px 0 40px 0">
        <div class="my-card notice">
                <p class="my-title">검색 결과</p>
                <p class="my-text"><?=$result?></p>
                <p class="my-text">문자 알림 신청자수 : <?=$people?>명</p>
                <div class="card-inner" style="height: 30px">
                    ...
                    <button type="button" id="addstar" class="btn btn-warning btn-sm" style="float:right" <?=$succ?>>즐겨찾기 추가</button>
                </div>
            </div>
        </div>
    </div>
    <script>
        $(document).ready(function(){
            ...
            $("#addstar").click(function(){
                location.href = "addstar.php?subj=<?=$subj?>&clss=<?=$clss?>";
            })
